  ...
  function clean(input) {
    ...
    return function(key, val) {
      if (!skip && typeof val === 'object' && input == val) {
        return '[Circular]';
      } else if (typeof val === 'function' || val instanceof Error ||
  ...
  function buildArray(data, obj=null) {
    if (data.type === 'Array') {
      ...
      for (var i = 0; i < data.value.length; i++) {
        if (data.value[i].type === 'string' &&
        data.value[i].value === '[Circular]' && obj !== null) {
          arr.push(obj);
        } else {
