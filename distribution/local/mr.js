...
mr.reduce = (args, cb) => {
  ...
  */
  ...
  store.get({key: null, gid: args.gid}, [args.mrid, 'reduce'], (e, keyValPairs) => {
    if (e) {
    ...
    for (let i=0; i<keyValPairs.length; i++) {
      ...
      reduceInputValues = Array.isArray(reduceInputValues) ? reduceInputValues : [reduceInputValues];
      const reducePromise = args.reduceFn(reduceInputKey, reduceInputValues);
      reducePromises.push(reducePromise);
    }

    Promise.all(reducePromises).then((reduceResults) => {
...
mr.append = (args, cb) => {
  ...
  Promise.all(promises).then((results) => {
    ...
    cb(new Error('Local.mr.append failed to store a key-value pair'), undefined);
  });
};
